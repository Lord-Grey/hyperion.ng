...
int main(int argc, char** argv)
{
	...
	#ifdef _WIN32
		const char* processName = "ambilightwifid.exe";
	#else
		const char* processName = "ambilightwifid";
	#endif
	...
	BooleanOption & resetPassword       = parser.add<BooleanOption> (0x0, "resetPassword", "Lost your password? Reset it with this option back to 'ambilightwifi'");
	...
	try
	{
		...
		HyperionDaemon* ambilightwifid = nullptr;
		try
		{
			ambilightwifid = new HyperionDaemon(userDataPath, qApp, bool(logLevelCheck));
		}
